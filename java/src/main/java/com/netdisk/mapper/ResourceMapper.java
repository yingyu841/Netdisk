...
public interface ResourceMapper {
    ...
    /**
     * 查询资源对应的回收站记录（不区分是否已恢复）。
     */
    RecycleEntry findAnyRecycleEntryByResourceId(@Param("resourceId") Long resourceId);
     ...
     * 复用历史回收站记录并重置为未恢复状态。
     */
    int reactivateRecycleEntry(RecycleEntry entry);
     ...
     * 查询资源 ACL 列表。
     */
    List<ResourceAcl> listAclByResourceId(@Param("resourceId") Long resourceId);

    /**
     * 删除资源指定主体 ACL。
     */
    int deleteAclByResourceAndSubject(
     ...
     * 删除资源全部 ACL。
     */
    int deleteAclByResourceId(@Param("resourceId") Long resourceId);

    /**
     * 新增资源 ACL。
     */
    int insertResourceAcl(ResourceAcl acl);
